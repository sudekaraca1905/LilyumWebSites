...
export function WorkshopCard({
  ...
  return (
    <Link
      href={`/atolyeler/${slug}`}
      className="group relative block overflow-hidden rounded-[1.75rem] border border-navy/10 bg-white/70 p-6 shadow-soft transition hover:-translate-y-1 hover:border-coral/40 hover:bg-white"
    >
      <div className="absolute -right-8 -top-8 h-28 w-28 rounded-full bg-gold/20 blur-2xl transition group-hover:bg-coral/25" />
      <p className="text-xs font-semibold uppercase tracking-[0.18em] text-coral">Atölye</p>
      <h3 className="mt-3 font-display text-2xl text-navy transition group-hover:text-coral-deep">
        {title}
      </h3>
